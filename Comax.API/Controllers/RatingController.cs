...
namespace Comax.API.Controllers
{
    ...
    public class RatingController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetMyRating(int comicId)
        {
            var userIdClaim = User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier);
            if (userIdClaim == null) return Ok(new { score = 0 }); // Chưa đăng nhập thì là 0

            int userId = int.Parse(userIdClaim.Value);
           
            var rating = await _unitOfWork.Ratings.GetUserRatingAsync(userId, comicId);

            return Ok(new { score = rating?.Score ?? 0 });
        }
    }
}